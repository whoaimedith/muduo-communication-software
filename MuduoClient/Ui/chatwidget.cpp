...
void ChatWidget::SendClicked(bool checked_)
{
    ...
    bool _bRet = _pClient->sendChatMessage(QString(m_nByteSource).toStdString(), QString(m_nByteDest).toStdString(), _strText.toStdString());
    assert(_bRet);
    ...
    else
    {
        ...
        {
            Message _nMessage;
            _nMessage.m_nContent =QString( _nByteText).toStdString();
